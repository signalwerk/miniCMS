...
`datetime`, `number`, `image`, `reference`, and `uuid`. Image uploads are
...
The API provides config, collection lists, record CRUD/rename, and media upload
under `/api`. Production serves the built editor from this package, never from
the consumer’s `admin/` directory.
