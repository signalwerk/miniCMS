...
test("serves the package-owned configuration editor schema", async () => {
  await withServer(async (baseUrl) => {
    const response = await fetch(`${baseUrl}/api/configuration-editor`);
    assert.equal(response.status, 200);
    const editor = await response.json();
    assert.equal(editor.sections.node_types.item_type, "node_type");
    assert.equal(editor.sections.collections.item_type, "collection");
    assert.deepEqual(
      editor.types.field.fields.widget.options
        .map((option) => option.value)
        .filter((value) => ["select", "object", "list"].includes(value)),
      ["select", "object", "list"]
    );
  });
});

test("validates and atomically replaces cms.config.yml", async () => {
  await withServer(async (baseUrl, rootDir) => {
    const config = await fetch(`${baseUrl}/api/config`).then((response) =>
      response.json()
    );
    config.site.name = "Edited site";
    config.node_types.page.fields.metadata = {
      label: "Metadata",
      widget: "object",
      fields: {
        theme: {
    ...
    assert.equal((await saved.json()).config.site.name, "Edited site");

    const source = await fs.readFile(
      path.join(rootDir, "cms.config.yml"),
      "utf8"
    );
    assert.match(source, /name: Edited site/);
    assert.match(source, /widget: object/);
    assert.match(source, /widget: list/);

    const invalidConfig = structuredClone(config);
    invalidConfig.node_types.page.fields.metadata = {
      label: "Broken object",
      widget: "object"
    };
    ...
    assert.match((await invalid.json()).message, /fields as a mapping/);

    const current = await fetch(`${baseUrl}/api/config`).then((response) =>
      response.json()
    );
    assert.equal(current.node_types.page.fields.metadata.label, "Metadata");
  });
});
